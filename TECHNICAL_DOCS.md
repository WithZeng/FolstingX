...
└──────────┬───────────────────────────┬──────────────────┘
           ...
           ▼                           ▼
┌──────────────────────┐  ┌───────────────────────────────┐
│  FolstingX 面板后端  │  │     节点 Agent (go-gost)      │
│  (Go/Gin)            │  │  ┌─────────────────────────┐  │
│  ┌────────────────┐  │  │  │   gost v3 转发引擎     │  │
│  │ REST API       │  │←→│  │   WebSocket 加密通信    │  │
│  │ /api/v1        │  │  │  │   自动接收面板指令      │  │
│  ├────────────────┤  │  │  └─────────────────────────┘  │
│  │ WebSocket Hub  │  │  └───────────────────────────────┘
│  │ /ws/monitor    │  │         ↑ 可部署多个节点
...
│  ├────────────────┤  │
│  │ 隧道管理       │  │  链路拓扑:
│  │ Tunnel/Chain   │  │  Entry → Relay → ... → Exit → Target
│  ├────────────────┤  │
│  │ 转发引擎管理   │  │
│  │ ForwardMgr     │  │
│  ├────────────────┤  │
...
### 核心概念 (参照 flux-panel 架构)

| 概念 | 说明 |
|------|------|
| **Node** | 物理/虚拟机器，运行 go-gost Agent，通过 WebSocket 连接面板 |
| **Tunnel** | 隧道，定义流量路径。Type=1 端口转发(直连)，Type=2 链式中转(多跳) |
| **ChainTunnel** | 链路节点，定义隧道中每一跳：Entry(入口) → Relay(中继) → Exit(出口) |
| **Forward** | 转发规则，用户在隧道上创建的具体端口映射 |
| **ForwardPort** | 端口分配，记录每个转发在各节点上占用的端口 |
| **AgentHub** | 面板端管理所有 Agent WebSocket 连接，下发 gost 配置指令 |
